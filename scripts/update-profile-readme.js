...
function updateProfileReadme() {
  try {
    ...
    if (!hasStartMarker || !hasEndMarker) {
      ...
      console.log(
        "Please add the following markers where you want the blog posts to appear:",
      );
    ...
    console.log(
      "✅ Profile README.md updated successfully with latest blog posts:",
    );
    posts.forEach((post) => {
...
function commitAndPush(profileDir) {
  try {
    ...
    // Configure git user
    const gitName = process.env.GITHUB_ACTIONS
      ? "GitHub Action"
      : "Blog Auto-Update";
    const gitEmail = process.env.GITHUB_ACTIONS
      ? "action@github.com"
      : "action@github.com";
    ...
    execSync('git commit -m "📰 Update README with latest blog posts"', {
      cwd: profileDir,
    });
...
if (require.main === module) {
  ...
  if (result.success) {
    ...
    if (process.env.GITHUB_ACTIONS || process.env.FORCE_PUSH) {
      ...
      console.log(
        "💡 Skipping git push (not in GitHub Actions). Set FORCE_PUSH=1 to force push locally.",
      );
    }
  }
